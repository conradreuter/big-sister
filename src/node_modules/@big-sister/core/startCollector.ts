...
export default function startCollector(
  ...
  function getCurrentPresence(): Presence {
    const presence: Presence = { presentIdentifiers: [], timestamp: new Date() }
    for (const [identifier, isPresent] of cache) {
      if (isPresent) presence.presentIdentifiers.push(identifier)
    }
    return presence
  }
}
