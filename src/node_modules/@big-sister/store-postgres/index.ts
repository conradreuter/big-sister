import { Identifier, Interval, StoredPresence, StorePlugin } from '@big-sister'
...
const plugin: StorePlugin<{}> = async (options) => {
  ...
  await setupDatabase()
  return { becomePresent, stayPresent, becomeAbsent, readPresence }
  ...
  async function readPresence(timeframe: Interval): Promise<StoredPresence[]> {
    ...
    const presences: { [identifier: string]: StoredPresence } = {}
    for (const { identifier, interval_from: from, interval_to: to } of results) {
      const presence = presences[identifier] || (
        presences[identifier] = { identifier, presenceIntervals: [] }
      )
      presence.presenceIntervals.push({ from, to })
    }
